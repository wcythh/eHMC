###eHMCu
eHMCu <- function(L_emp,epsilon,M,theta,d,Niter,feature_names,U,grad_U,Leapfrog)
{
  ...
  time_eHMCu <- proc.time()
  for(i in 2:Niter)
  {
    L <- sample(L_emp,size=1)
    r <- sample(1:L,size=1)
  ...
  time_eHMCu <- proc.time() - time_eHMCu
  print(c(sum(ratio)/Niter,time_eHMCu))
