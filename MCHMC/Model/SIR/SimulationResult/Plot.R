...
eHMC_KS <- matrix(0,nrow=40,ncol=4)
eHMCq_KS <- matrix(0,nrow=40,ncol=4)
eHMCu_KS <- matrix(0,nrow=40,ncol=4)
MCHMC_KS <- matrix(0,nrow=40,ncol=4)
Result <- get(load(file="Result.RData"))
for(i in 1:40)
{
  rt <- Result[[i]]
  NUTS_KS[i,] <- rt$NUTS_KS[Index2]
  eHMC_KS[i,] <- rt$eHMC_KS[Index2]
  eHMCq_KS[i,] <- rt$eHMCq_KS[Index2]
  eHMCu_KS[i,] <- rt$eHMCu_KS[Index2]
  MCHMC_KS[i,] <- rt$CVHMC_KS[Index2]
}
...
####### Mean KS
ESSTransformed <- NULL
ESSTransformed <- rbind(ESSTransformed, cbind(0, apply(NUTS_KS, 1, mean)))
ESSTransformed <- rbind(ESSTransformed, cbind(1, apply(eHMC_KS, 1, mean)))
ESSTransformed <- rbind(ESSTransformed, cbind(2, apply(eHMCq_KS, 1, mean)))
ESSTransformed <- rbind(ESSTransformed, cbind(3, apply(eHMCu_KS, 1, mean)))
ESSTransformed <- rbind(ESSTransformed, cbind(4, apply(MCHMC_KS, 1, mean)))
