...
iter0 <- 25e3
warmup0 <- 5e3
fit <- stan("irt_2pl_log.stan",data=data, chains=40, iter=iter0, warmup = warmup0,
            init = "random",
            #init = init0,
...
for(k in 1:40)
{
  ...
  NUTS_Summary <- list(Xsamp=Xsamp_NUTS, Comp=CompNUTS, epsilon=epsilon0,
...
MixSampler0 <- function(k)
{
  ...
  LearnL_Summary <- list(Xsamp=fit0$Xsamp,L_emp=fit0$L_emp, accept=fit0$ratio/fit0$Niter)
...
MixSampler1 <- function(k)
{
  ...
  eHMC_Summary   <- list(Xsamp=fit1$Xsamp,Comp=fit1$Comp, epsilon=fit1$epsilon,
...
MixSampler2 <- function(k)
{
  ...
  eHMCq_Summary  <- list(Xsamp=fit2$Xsamp,Comp=fit2$Comp, epsilon=fit2$epsilon,
...
MixSampler3 <- function(k)
{
  ...
  eHMCu_Summary  <- list(Xsamp=fit3$Xsamp,Comp=fit3$Comp, epsilon=fit3$epsilon,
...
rm(Result0)
#source("DefinedFunctions/UandGradU_byhand.R")
...
for(k in 1:40)
{
  NUTS_Summary <- get(load(file=paste("NUTS_Summary",toString(k),".RData",sep="")))
  LearnL_Summary <- get(load(file=paste("LearnL_Summary",toString(k),".RData",sep="")))
  eHMC_Summary <- get(load(file=paste("eHMC_Summary",toString(k),".RData",sep="")))
  eHMCq_Summary <- get(load(file=paste("eHMCq_Summary",toString(k),".RData",sep="")))
  eHMCu_Summary <- get(load(file=paste("eHMCu_Summary",toString(k),".RData",sep="")))
  
  Result[[k]] <- list(NUTS_Summary=NUTS_Summary, LearnL_Summary=LearnL_Summary,
