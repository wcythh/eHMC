...
iter0 <- 3e3
warmup0 <- 2e3
fit <- stan("Logit.stan",data=data, chains=3, iter=iter0, warmup = warmup0,
            init = "random",save_dso = FALSE, verbose=TRUE,algorithm = "NUTS",
            control = list(adapt_engaged=TRUE,stepsize=0.01,adapt_delta=0.9,
...
MixSampler <- function(k)
{
  ...
  fit1 <- eHMC( fit0$L_emp,epsilon0,M0,init0,d,2e3,feature_names0,U,grad_U,Leapfrog)
  fit2 <- eHMCq(fit0$L_emp,epsilon0,M0,init0,d,2e3,feature_names0,U,grad_U,Leapfrog)
  fit3 <- eHMCu(fit0$L_emp,epsilon0,M0,init0,d,2e3,feature_names0,U,grad_U,Leapfrog)
...
Result <- parLapply(cl, 1:2, function(zz) MixSampler(zz))
